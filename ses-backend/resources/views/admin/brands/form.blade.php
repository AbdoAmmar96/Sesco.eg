...
@section('title', $item->exists ? 'Edit Brand' : 'Add Brand')

@section('content')
    <a href="{{ route('admin.brands.index') }}" class="text-sm font-semibold text-slate-500 hover:text-navy-900">← Back to brands</a>
    ...
    <form method="POST" action="{{ $item->exists ? route('admin.brands.update', $item) : route('admin.brands.store') }}"
        ...
        <x-input name="name" label="Brand Name" :value="$item->name" required />
        ...
        <div class="flex gap-3 pt-2">
            ...
            <a href="{{ route('admin.brands.index') }}" class="rounded-lg border border-slate-300 px-5 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50">Cancel</a>
        </div>
    </form>
@endsection
